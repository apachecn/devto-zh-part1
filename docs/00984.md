...
# [](#storage-cost-calculation)存储成本计算
...
## [](#amazon-simple-storage-service-s3)亚马逊简单存储服务(S3)
...
## [](#amazon-elastic-file-system-efs)亚马逊弹性文件系统(EFS)

EFS 是 Amazon EC2 实例的可扩展网络文件存储，您只需为文件系统中使用的存储量付费。

## [](#amazon-elastic-block-store-ebs)亚马逊弹性街区商店(EBS)
...
## [](#amazon-ec2-instance-storage)亚马逊 EC2 实例存储

Amazon EC2 实例的临时块存储卷，本地实例存储卷上的数据存储不收取额外费用。但是，跨区域和可用性区域的数据传输会产生数据传输费用。

## [](#aws-storage-gateway)AWS 存储网关
...
# [](#estimate-and-monitor-usage-and-costs)估计和监控使用情况和成本
...
[![AWS Billing Stats](../Images/d9eaef7700b2f34e20b610c82fd20340.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--PPtLNFvw--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/8h1pee739tpnhetmb91z.png)
