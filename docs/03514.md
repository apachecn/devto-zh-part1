...
> 原文：<https://dev.to/rob__race/testing-stripe-with-rails-and-rspec-6o8>

[![](img/52d0a6858ec47296831b76de318fbe8b.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--wBhc3d48--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/2000/1%2A4nckzHj32Zaum7Qop2RlOg.png)
...
如果这是您喜欢的内容，那么您将能够从我的书[在 Rails 5](https://buildasaasappinrails.com/) 中构建 SaaS 应用中看到完整的实现(包括全面的测试套件)。这本书指导你从卑微的开始，通过部署一个应用到生产。这本书快完成了，你可以马上拿到免费的一章！
...
无论如何，点击`Sign Up`会带你到一个`plans#index`页面。在该页面上，您可以点击一个链接，选择开始试用的计划。一旦完成，该计划将通过注册过程的继续，最终在一个服务对象中使用，该服务对象将条带创建委托给以前的同一个`CreationService`类。

如果条带订阅有任何问题，工作流不会在`root_path`结束，也不会失败测试。您甚至可以更进一步，让该功能访问您的应用程序的计费部分，并确保所选择的计划是显示为`current`的计划。
...
在进入条纹网钩的规范之前。让我来为这位 [StripeEvent](https://github.com/integrallis/stripe_event) gem 欢呼吧。作为一名优秀的软件维护者，我喜欢确保 gem 安装是值得的。Stripe_event 是一个设置 WebhooksController 的小型 gem，以及一个添加了一点元编程的小型类文件集。它使用 ActiveSupport::Notifications 将事件传递给处理代码的您(您可以在块之间进行选择，或者向您选择的类发送. call)。
...
默认情况下，StripeMock 实际上在 gem 结构内的 JSON fixtures 中保留了大多数 webhook 事件的副本。但是，对于每种类型的事件，您可以覆盖特定的属性。在这里，为了确保事件为正确的客户带来订阅，它们在创建模拟事件的第二个参数中被覆盖。
