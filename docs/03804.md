...
## [](#method-signatures-are-not-real-code)方法签名不是真正的代码
...
## [](#parameter-types-are-already-described-in-the-phpdoc)参数类型已经在 phpdoc 中描述过
...
## [](#long-method-signatures-are-an-alarm-signal-about-having-too-many-arguments)长方法签名是参数过多的警告信号
...
## [](#switching-to-multiline-signatures-will-be-the-source-of-many-conflicts)切换到多行签名将是许多冲突的根源

我是一些不使用多行签名的库的维护者。猜猜大部分冲突发生在哪里？没错，它们发生在很长的线上，现在实际上只有长签名才会发生。所以从长远来看，这些冲突也许是值得的。

[![long lines are a recipe for git conflicts](../Images/feee9f7c3c0bd79bb1f18b8fe6d34afb.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--c-6HLk0G--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i.imgflip.com/1xv95r.jpg)

此外，考虑到切换到多行签名不必一蹴而就:只要允许切换到它们，并在它们太长时在新创建的方法上强制它们。

## [](#multiline-signatures-look-weird)多行签名看起来很奇怪
