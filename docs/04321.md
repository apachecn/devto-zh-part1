...
## [](#1-use-the-raw-ltbuttongt-endraw-element-for-anything-that-users-click-on)1。对用户点击的任何东西使用`<button>`元素。
...
## [](#2-plan-for-common-keyboard-interactions)2。计划常见的键盘交互。
...
## [](#3-manage-aria-states)3。管理 ARIA 状态
...
### [](#get-the-first-focusable-element)获取第一个可聚焦元素。
...
### [](#ensure-that-users-can-easily-close-the-modal-via-the-keyboard-when-it-is-open)确保用户可以在模态打开时通过键盘轻松关闭模态

我们在前面讨论规划通用键盘交互时已经部分解决了这个问题。当您希望用户能够退出组件时，这是一个很好的例子。

您还可以在模式和站点内容之间添加一个覆盖层，该覆盖层可以通过关闭模式的点击事件来点击和聚焦。

### [](#when-the-modal-closes-return-focus-to-the-element-that-was-active-when-the-modal-opened)当模态关闭时，将焦点返回到模态打开时活动的元素。
...
### [](#trap-the-tab-and-shift-tab-inside-the-modal)陷印 TAB 和 SHIFT + TAB 在模态内
