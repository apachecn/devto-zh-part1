...
## [](#gnew-create-a%C3%A2-new-branch)gnew -创建新的分支
...
## [](#gswitch-quickly-checkout-branches)gswitch -快速结账分店
...
## [](#aliases-lookup-what-aliases-you-have)别名-查找您有哪些别名

`aliases = !git config --get-regexp 'alias.*' | colrm 1 6 | sed 's/[ ]/ = /'`

我*所以*需要在 bash 中重新创建这个，我很难记住我创建的所有别名(T2 有很多)。ðŸ˜

## [](#cam-commit-while-adding-files-and-attach-a-message)cam -添加文件时提交并附加消息
...
## [](#deletelocal-clear-out-old-branches-quicker)删除-本地-更快地清除旧分支

`delete-local = branch -d`

不知何故，我更容易记住`git delete-local`而不是`git branch -d`。

## [](#deleteremote-clear-out-remote-branches-quicker)删除-远程-快速清除远程分支

`delete-remote = "!sh -c 'git push origin :$1' -"`

注意:这并不是问你在做之前是否知道自己在做什么，所以要小心！

## [](#branches-get-a-list-of-all-of-your-local-branches)【分支机构】-获取所有本地分支机构的列表

`branches = "!sh -c 'git branch -a | grep -v remotes'"`

如果您还想要一个远程分支的列表，您可以去掉`grep -v remotes`。这是“自然语言”的另一个别名，输入`git branches`感觉更好。

## [](#editunmerged-amp-addunmerged-fix-merge-conflicts-like-a-boss)编辑-未合并&添加-未合并-像老板一样修复合并冲突
