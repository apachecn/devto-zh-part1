...
# [](#setting-up-coderunner)设置 CodeRunner

*   首先进入设置(`⌘;` ) →语言，在左侧列表中搜索`PHP`。
*   用列表![duplicate the PHP language](../Images/bd88252be1821f44eb53aed58e54a700.png)下方的设置按钮复制`PHP`
...
*   在工具栏中，有一个下拉字幕语言。选择您新创建的标记为`PHP 7.1 via Docker`的 PHP 环境。![Choose PHP 7.1 via Docker from Languages dropdown](../Images/db4a6805328394304292c2788326fb93.png)

# [](#setting-up-docker-for-mac)为 Mac 设置 Docker

在我们开始之前，我们必须允许 Mac 版的 Docker 挂载(CodeRunner 放置临时文件的目录。为了做到这一点

*   通过菜单项![Open Docker for Mac Preferences via its menu item](../Images/c5f27b6e510ecfd8ff6bee8a3440ebf8.png)打开 Mac Preferences…的 Docker
*   转到`File Sharing`选项卡并点击`+`按钮
*   这将打开系统打开对话框。你打开哪个目录并不重要，所以你只需点击右下角的打开按钮
*   新创建列表项必须双击。现在你可以修改它了
*   输入`/var/folders/pc`并按回车键(↩) ![Docker for Mac after adding /var/folders/pc to the file sharing tab](../Images/e68bd26d7a2f46785e470914cca84dc2.png)
...
然后你会得到你的 PHP 脚本的输出: [![Running PHP 7.1 with CodeRunner via Docker](../Images/6616da98dbe5950e14037737182bd552.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--ycQbLCtu--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/jchf51jyp4066ljqlzij.png)
