...
## [](#installing-nginx-as-a-reverse-proxy)安装 Nginx 作为反向代理

既然我们已经了解了什么是反向代理，以及为什么我们可能想要使用它，那么让我们看看如何将 Nginx 安装为反向代理。

### [](#step-1-installing-nginx)第一步:安装 Nginx
...
#### [](#ssl-defaults)SSL 默认值

首先，让我们看看设置 SSL 证书和所需的 dhparam 文件。

##### [](#getting-an-ssl-certificate)获取 SSL 证书

我们将看看设置让我们加密，一个免费的认证机构。

###### [](#step-1-installing-the-certbot-lets-encrypt-client)步骤 1 -安装证书机器人让我们加密客户端
...
###### [](#step-2-obtaining-a-certificate)第二步-获取证书
...
###### [](#step-3-updating-diffiehellman-parameters)步骤 3 -更新 Diffie-Hellman 参数
...
###### [](#step-4-setting-up-auto-renewal)步骤 4 -设置自动续费
...
##### [](#creating-the-ssldefaultsconf-snippet)创建 ssl-defaults.conf 片段
...
#### [](#default-server-block-template)默认服务器块模板
