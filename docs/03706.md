...
> 原文:[https://dev . to/jessaminmorgan/3-为企业移动应用构建大数据后端的挑战-76m](https://dev.to/jessaminmorgan/3-challenges-of-building-a-big-data-backend-for-your-enterprise-mobile-app-76m)

[![Big Data Backend for Your Enterprise Mobile App](../Images/127221dda971344808ef078bc58d30d2.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--bvSLdVEF--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/4i5g2t66hsclq1ux5pmq.png)

大多数应用程序通过提供互联网接入并向用户提供关于他们的问题和行动的实时反馈来带来价值。很少有企业应用是独立的，不需要互联网连接。这些仅仅是静态信息的显示。这就是为什么值得考虑后端和 API 来丰富企业应用程序功能的原因。

## [](#understanding-the-dark-world-of-the-backend)了解后台的黑暗世界

对于应用程序开发人员来说，后端可能会令人生畏，因为它需要一套不同的技能来处理受加密和安全规则控制的服务器和数据库。在处理大数据时，甚至还有更多要求，比如与处理 Hadoop 框架相关的要求。

### [](#delegate-to-server)委托给服务器

由于与服务器相比，移动设备的资源有限，所以最好将所有计算工作转移到一台强大的机器上，并且只将结果传达给用户。对于开发人员来说，这有助于为不同的平台构建应用程序，并在最后集成它。

### [](#plan-for-offline-backup)计划进行离线备份

用户在使用该应用程序时可能会遇到信号弱的问题或根本没有互联网连接。一个设计良好的软件可以创建本地数据备份，并在链接可用时与服务器重新同步。如果该应用程序可以处理大数据，离线存储的数据量不能超过设备的容量，这一点非常重要。

### [](#address-synchronization-issues)解决同步问题
...
## [](#selecting-the-right-query-tool-sql-vs-nosql)选择合适的查询工具:SQL vs. NoSQL
...
## [](#solutions-to-security-issues)安全问题的解决方案
...
## [](#is-backend-as-a-service-baas-a-viable-solution)后端即服务(BaaS)是可行的解决方案吗？
