...
## [](#-raw-create-table-like-endraw-doesnt-copy-views)`CREATE TABLE LIKE`不抄观点
...
## [](#-raw-create-table-like-endraw-copies-everything-from-the-structure-except-constraints)`CREATE TABLE LIKE`从结构中复制一切，约束除外
...
## [](#-raw-mysqldump-endraw-might-take-forever-to-run-across-a-network)`mysqldump`可能要花一辈子才能跑过一个网络
...
## [](#-raw-mysqldump-endraw-might-ignore-your-foreign-key-order)`mysqldump`可能会忽略你的外键顺序
...
### [](#verify-if-we-had-a-local-copy-of-the-original-db-if-the-answer-is-yes)验证我们是否有原始数据库的本地副本。如果答案是肯定的…
...
### [](#if-we-had-to-connect-to-an-external-db)如果我们必须连接到外部数据库…
