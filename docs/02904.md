...
### [](#identifyargs-callbackerr-output)identify(args，callback(err，output))
...
### [](#readmetadatapath-callbackerr-metadata)readMetadata(路径，回调(错误，元数据))

虽然使用了`identify`命令，但是我们将结果作为`metadata`对象返回。

### [](#convertargs-callbackerr-stdout-stderr)convert(args，callback(err，stdout，stderr))

使用`convert`命令。 这也是`args`保持自变量不变。
[转换|图像命令参考](http://image-magick.com/2014/09/29/convert/)

### [](#resizeoptions-callbackerr-stdout-stderr)resize(options, callback(err, stdout, stderr))
