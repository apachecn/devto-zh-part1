...
> 原文：<https://dev.to/rob__race/integrating-bootstrap-into-rails-5>

## …以及如何使用引导主题

Bootstrap 是一个 HTML/CSS/JS 框架，可以快速简单地构建一个响应性网站。Bootstrap 使用网格布局系统，将页面分成若干行，每行包含 12 列。这现在允许您以一致的方式组织您的标记。

* * *

*注意:本教程摘自我即将出版的书[在 Rail 5](https://buildasaasappinrails.com) 上用 Ruby on Rails 构建一个 SaaS 应用。这本书将指导你从卑微的开始，通过部署一个应用到生产。如果你觉得这类内容有价值，这本书现在就在预售中！我把注释放在这里是为了不破坏 Dev.to 的元信息自动化的东西。*

* * *

Bootstrap 已经存在了很多年，也开发了一个主题生态系统。有高级和免费主题可供选择。就我个人而言，在构建 SaaS 应用程序时，我喜欢将 Inspinia 作为高级主题，或者将 AdminLTE 作为免费主题。

出于本文的目的，由于它的开源和免费性质，我们将使用 AdminLTE 主题。随意使用任何其他主题。尽管类名和标记结构可能会因您所使用的主题而有所不同。
这篇文章还假设你已经用 rails new app_name 生成了一个新的 rails 应用，并在默认端口 3000 上运行一个 Rails 服务器。

### 网格快速概览

在将 Bootstrap 集成到您的应用程序之前，让我们更详细地了解一下网格系统。如前所述，网格是一个 12 列系统，它响应视口或设备宽度。当您在网格中使用
...
在示例中，您可能会注意到列类名包含字母 lg。这代表“大”,意味着这些列引用了大视口尺寸。有四种尺寸，每种对应最大的像素宽度。超小型手机(< 768 像素)。小型设备平板电脑(≥768px)。中型设备台式机(≥992 像素)。大型设备台式机(≥1200px)。
...
要深入了解网格系统和 Bootstrap 的其他 CSS 功能，可以访问文档。

### 第一个控制器，视图

如果这是一个新的应用程序，在您开始实现布局之前，我们需要实现一个路由和控制器来绕过默认的欢迎页面。您将在应用程序中使用控制器。

让我们停下来提一下，Rails 自带了内置的命令行生成器来生成控制器、模型和邮件。以及生成数据库迁移文件或甚至生成所有这些组合的支架。在我们的例子中，让我们关注控制器生成器。这个生成器的一般格式是 rails 生成控制器名[action action] [options]。因此，我们的控制器生成的命令将看起来像:
...
让我们对路由文件进行一次快速编辑，以定义默认路由。我们将在路线文件的底部添加根目录:“activity#mine”，如下所示:
...
如果您正在使用 AdminLTE Bootstrap 主题并遵循该主题，那么您应该下载它们。你可以从这个链接或者主题预览的支持页面下载它。
...
既然所需的文件已经存在，现在您将指示 Rails 应用程序使用这些文件。首先将`assets/stylesheets/application.css`文件改为只需要它自己和一个新的 style.scss 文件。
...
[![Styled](img/41b6bf4a08b3a790ea4aba423d0b2df0.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--XBR4Jwb3--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/2000/1%2A--UkS93-c7jBiszdQcljFw.png)
...
这里有几点需要注意。首先，您可能会注意到 head 部分中以=开头的行。这是 slim templates 包含 ruby 输出的方式。这与在模板中使用-不同。单个连字符用于包含 ruby，但是没有输出，就像 if 语句一样。头部的输出包括 Rails 资产 javascript 和 CSS。我们还通过最后一个样式表包含帮助器包含了 FontAwesome 图标。你可能还会注意到正文中一些非常特殊的标记。这里，您将使用 AdminLTE 特定的类和 HTML 层次结构来设计 AdminLTE 主题中的应用程序。

最后，您将看到另外两个助手。`= render`助手，包括其他瘦文件。然后，`= yield`是 Rails 的模板方法，用于包含来自控制器的模板和被调用的动作。

在深入研究布局中的特定部分之前，本部分要做的最后一件事是修改`application.js`以包含主题工作所需的必要 javascript。您将从主题的文件夹中复制另一个文件，然后确保它在您的项目中被引用。
...
此外，您还需要主题文件夹中的第三方 javascript 文件:
...
您可能会注意到这个文件被复制到不同的位置。在 Rails 应用程序中，将第三方 javascript 放在供应商文件夹中被认为是最佳实践。
...
生成新的 Rails 应用程序时，默认包含前三行。这些行包括基本的 jQuery、工具和 Turbolinks。您将添加的行包括来自 bootstrap-sass gem 的 Bootstrap。还包括 AdminLTE 主题使用的`slimscroll`插件。最后一行是另一个 Rails 默认值。这意味着该文件将包含`app/assets/javascripts`文件夹及其子文件夹中的任何 javascript 文件。这一行就是复制的`app.js`文件和其他文件被包含进来的原因。

### 顶部导航

关于此应用程序中的导航，将有两个主要部分。顶部的导航栏和屏幕左侧的导航边栏。

在应用程序文件中，导航文件是通过在导航文件夹中呈现一个细长模板(= render ' layouts/navigation/layout ')来包含的。在这个文件中，您将包括顶部导航和侧面导航模板:
...
包含的第一部分是标题或顶部导航栏模板。这篇文章中采用的布局方法将包括示例标记。稍后，您将添加、移除或更改标记，以满足正在构建的应用程序的期望。下面是标题部分:
...
包含的第二个模板是侧边栏或侧面导航栏模板。这部分将采用相同的示例标记方法。
...
布局的最后一个主要部分是页脚。这将是简单而简短的。再次，这将是另一个部分。同样，该部分将位于导航文件夹中。下面是简短的标记:
...
转到 [http://localhost:3000](http://localhost:3000) ，您现在应该会看到一个如下所示的页面，或者如果您偏离了主题或布局风格，会看到类似的页面:

[![Layout](img/64fa05e31b21080d977fa4f46c78ac44.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--S9nD2u69--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/1600/1%2ALWB2Ag3pu6CnL00Hz8s0kw.png)
