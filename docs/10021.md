...
> 原文:[https://dev . to/jcolemorrison/authorized-resources-and-database-migrations-with-strong loops-loopback](https://dev.to/jcolemorrison/authorized-resources-and-database-migrations-with-strongloops-loopback)
...
#### [](#1-create-a-raw-code-endraw-directory-and-navigate-to-it-in-your-command-line)1)创建一个`code`目录，并在命令行中导航到该目录

`$ mkdir code && cd code`

可能没必要提怎么做。

#### [](#2-create-a-folder-inside-of-raw-code-endraw-called-raw-devimages-endraw-and-another-within-that-called-raw-strongloop-endraw-)2)在`code`中创建一个名为`dev-images`的文件夹，并在其中创建一个名为`strongloop`的文件夹
...
#### [](#3-create-the-dockerfile-inside-of-raw-codedevimagesstrongloop-endraw-)3)在`code/dev-images/strongloop`中创建 Dockerfile
...
#### [](#4-input-the-following)4)输入以下内容:
...
#### [](#5-build-the-docker-image)5)构建 Docker 图像
...
#### [](#6-create-the-raw-dockercompose-endraw-file)6)创建`docker-compose`文件
...
#### [](#7-scaffold-out-the-strongloop-application)7)脚手架退出 Strongloop 程序
...
#### [](#10-open-up-our-raw-dockercomposeyml-endraw-file-and-modify-it-to-reflect-the-following)10)打开我们的`docker-compose.yml`文件，并修改它以反映以下内容:
...
#### [](#11-in-the-raw-code-endraw-directory-create-a-new-file-raw-env-endraw-and-input-the-following)11)在`code`目录下创建一个新文件`.env`并输入以下内容:
...
#### [](#12-in-our-raw-code-endraw-directory-run-the-following-to-up-the-api-server-and-mysql-service)12)在我们的`code`目录中，运行以下命令启动 api 服务器和 mysql 服务:
...
#### [](#13-install-the-raw-loopbackconnectormysql-endraw-package)13)安装`loopback-connector-mysql`包
...
#### [](#14-in-the-raw-code-endraw-directory-run-the-following)14)在`code`目录下，运行以下命令:
...
#### [](#15-create-the-raw-widget-endraw-model-by-running-the-following)15)通过运行以下程序创建`Widget`模型:
...
#### [](#16-relate-the-raw-widget-endraw-and-raw-client-endraw-via-a-raw-hasmany-endraw-relation)16)通过`hasMany`关系将`Widget`和`Client`联系起来:
...
#### [](#17-head-back-over-to-raw-localhost3002explorer-endraw-)17)头回过头来`localhost:3002/explorer`
...
#### [](#19-run-the-migration-script)19)运行迁移脚本
...
#### [](#20-create-a-raw-widget-endraw-)20)创造一个`Widget`
...
[![](../Images/90c862f4c03f99dbce543fad09a8b5a1.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--pxwIftUS--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://s3-us-west-1.amazonaws.com/www.jcolemorrison.com/blog/posts/strongloop-loopback-docker-1-29-2017/step-20.png)

c)点击`Try it out!`，将创建`Widget`。

现在解决用新模式更新表的问题。

#### [](#21-navigate-to-raw-servermodelswidgetjson-endraw-and-add-the-following-property)21)导航到`server/models/widget.json`并添加以下属性:
...
#### [](#22-head-back-to-raw-localhost3002explorer-endraw-and-attempt-the-following-raw-widget-endraw-)22)返回`localhost:3002/explorer`并尝试以下`Widget`:
...
#### [](#23-create-a-new-file-at-raw-serverbootbasemigrationjs-endraw-)23)在`server/boot/base.migration.js`创建一个新文件
...
#### [](#24-create-a-new-file-at-raw-serverbootcustommigrationjs-endraw-)24)在`server/boot/custom.migration.js`创建一个新文件
...
#### [](#25-head-back-to-raw-localhost3002explorer-endraw-and-create-the-huge-widget)25)返回`localhost:3002/explorer`并创建巨大的部件
...
[![](../Images/71472687a2f0adb438a60891ff92b45e.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--vRn8EQPJ--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://s3-us-west-1.amazonaws.com/www.jcolemorrison.com/blog/posts/strongloop-loopback-docker-1-29-2017/step-25.png)
...
#### [](#26-head-over-to-the-raw-localhost3002explorer-endraw-)26)脑袋转到了`localhost:3002/explorer`
...
[![](../Images/974b9b425296f67e301f889004618880.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--CDZt9SU3--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://s3-us-west-1.amazonaws.com/www.jcolemorrison.com/blog/posts/strongloop-loopback-docker-1-29-2017/step-26.png)
...
#### [](#27-hop-over-to-raw-clientjson-endraw-and-add-the-following-object-in-the-raw-acls-endraw-array)27)跳到`client.json`并在`acls`数组中添加以下对象:
...
#### [](#28-head-back-to-raw-localhost3002explorer-endraw-and-raw-post-endraw-the-widget)28)头回`localhost:3002/explorer`和`POST`小部件
...
#### [](#29-open-up-raw-serverwidgetjson-endraw-and-add-the-following-acl)29)打开`server/widget.json`并添加以下 ACL:
