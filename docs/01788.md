...
> 原文:[https://dev . to/goshishah/xmlhttprequestxhr-not-send-to-server-on-before unload-event-d1l](https://dev.to/goshishah/xmlhttprequestxhr-not-send-to-server-on-beforeunload-event-d1l)

这是我在任何博客/网站上的第一篇文章，我不是一个好作家。我是初级 ASP.NET 软件工程师，最近我在 React 工作，负责前端开发。

昨天在窗口关闭时用 JavaScript 写了一个函数。函数的职责是向服务器发送 xhr 调用。

## [](#here-is-my-source-code)这里是我的源代码。
...
## [](#after-change-async-to-sync)将异步改为同步后
...
## [](#question-why-unlock-function-was-not-sending-the-call-to-the-server)问题:为什么解锁功能没有将调用发送到服务器？