...
### [](#making-sure-nothing-goes-wrong)确保一切顺利
...
#### [](#trick-1-testing-the-raw-where-endraw-clause)招数 1:测试`WHERE`子句

在运行`DELETE`或`UPDATE`之前，我们应该使用相同的 WHERE 子句运行`SELECT`。所以我们知道我们的目标是想要的数据集。

#### [](#trick-2-using-a-transaction)招数二:利用交易
...
#### [](#trick-3-having-a-backup-of-the-test-database)招数三:备份测试数据库

有测试数据库的备份很好。这可以是虚拟机的快照，也可以是数据库文件的快照。所以如果其他一切都失败了，你总是可以回到以前的状态。

### [](#making-sure-the-impact-is-minimal)确保影响最小
...
### [](#having-a-plan-to-fix-it)有计划地解决问题
