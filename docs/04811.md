...
> 原文:[https://dev . to/jreina/intro-to-fold-function-aka-reduce-or-aggregate](https://dev.to/jreina/intro-to-the-fold-function-aka-reduce-or-aggregate)
...
[![fold execution diagram](../Images/ac830c4e916983f0579e4bd052ffeafb.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--Rtygifvd--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/eqr7luaz5s8uligyllr9.png) 

这里，`f`是累加器函数。请注意，`f`的每个实例都有两个指向它的箭头。这意味着您提供的累加器函数必须接受两个参数:来自上次执行的值和来自数组的当前值。按照惯例，我倾向于使用`(memo, value)`作为我的参数名。

## [](#starting-off-slow)起步慢
