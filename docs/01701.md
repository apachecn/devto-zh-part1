...
> 原文:[https://dev . to/abelagoi/gentle-introduction-to-reactive-forms-in angular-d9m](https://dev.to/abelagoi/gentle-introduction-to-reactive-forms-inangular-d9m)

反应式表单是我们在代码的组件类部分中编写逻辑、验证和控制的表单，与模板驱动表单不同，模板驱动表单在模板中完成控制。反应式表单非常灵活，可用于处理任何复杂表单场景。我们编写更多的组件代码和更少的 html 代码，这使得单元测试更容易。

### [](#enabling-reactive%C2%A0forms)启用反应式表单
...
### [](#formcontrol-and-formgroup)表单控件和表单组
...
### [](#simplify-with-formbuilder)用表单生成器简化
...
### [](#displaying-validation-error)显示验证错误
...
### [](#disable-submit%C2%A0button)禁用提交按钮
