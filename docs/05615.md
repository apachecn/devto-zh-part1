...
> 原文:[https://dev.to/andrew565/which-array-function-when](https://dev.to/andrew565/which-array-function-when)
...
# [](#return-one-new-entry-for-every-existing-entry-raw-map-endraw-)为每个现有条目返回一个新条目:`map()`
...
# [](#return-a-new-array-with-only-some-of-the-existing-entries-raw-filter-endraw-)返回一个只有部分现有条目的新数组:`filter()`
...
# [](#return-one-new-thing-only-raw-reduce-endraw-)只返回一个新东西:`reduce()`
...
# [](#do-something-with-each-array-value-but-dont-return-anything-raw-foreach-endraw-)对每个数组值做一些事情，但不返回任何东西:`forEach()`
