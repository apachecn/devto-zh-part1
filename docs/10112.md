...
> 原文：<https://dev.to/rosstate/java-is-unsound-the-industry-perspective>

最近 [Nada Amin](https://twitter.com/nadamin) 和我发现 Java 和 Scala 不健全。我们将这一发现和相关讨论提交给了面向对象编程的学术会议 OOPSLA。很高兴看到纸张在工业界流传，但似乎也有很多混乱，我认为这是我们的错。这篇论文是为学术界写的，所以它没有从工业界的角度讨论这个问题。我现在正在赶一个截止日期，所以让我在这里写一篇针对行业的论文的节略版，只关注 Java。

## “不健全是什么意思？

大多数类型系统的目标是为一个良好类型的程序的行为提供某种保证。例如，在许多语言中，所有的内存访问“在类型良好的程序中是安全的”。如果一个类型系统实际上成功地提供了这种保证，那么它就是健全的。因此，非正式地说，如果一个类型系统确保了它的设计者想要它确保的东西，那么它就是健全的。这很像编程:如果一个程序做了它应该做的事情，它就是正确的。
...
当然，JavaScript 可以成功地执行这个程序，尽管它本质上是无意义的，但是 Java 的实现非常不同，允许这个代码真的会弄乱堆(或者甚至是栈，如果你喜欢一些 C/C++内存管理不当的错误)。

现在你可能已经知道 Java 的类型系统有一些不完善的地方。例如，由于历史原因，Java 有协变数组，进行代码类型检查:
...
因此，Java 的类型系统看起来就像预期的那样完善，尽管可能没有您希望的那么完善。自从泛型被引入以来，它被认为是“真正合理的，只要你不使用这些有意的后门”。

## “真不健全

> 我昨天在推特上发布了这个消息，但是由于某种原因它没有被发现——考虑一下这个程序:它编译了 w/o 错误并抛出了 ClassCastExc[pic.twitter.com/uRAleRKKHH](https://t.co/uRAleRKKHH)
> 
> — Joshua Bloch (@joshbloch) [January 21, 2017](https://twitter.com/joshbloch/status/822948565433466881)

这是我们最近流传的“真正不健康的例子”。它将一个`Integer`实例分配给一个`String`变量。根据 Java 规范，这个程序是良好类型的，根据 Java 规范，它应该毫无例外地执行*(不像前面提到的后门程序)并且*实际上*将`Integer`实例分配给`String`变量，违反了良好类型程序的预期行为。由此说明 Java 的类型系统是不健全的。*
...
3.  如果你的编译器确实进行了类型检查，然后你运行它，你会得到一个`ClassCastException`，这也与我的说法相矛盾。
...
因此，如果历史稍有不同，无论是 Sun 在添加泛型后决定放弃向后兼容性，还是 Sun 将泛型添加到 JVM 中，或者 Java 首先与泛型一起发布，都不会抛出这个异常。相反，我可以将任何内容转换成一个`int[]`并直接访问许多对象的原始字节，然后我可以用它来注入任意代码(使用[面向返回的编程](https://en.wikipedia.org/wiki/Return-oriented_programming)来绕过安全措施)。

## “但是没有人会写这个”

你可能永远不会写这个，你的同事也不会写这个。这种推理适用于许多情况。我实际上做这件事的[都](http://www.cs.cornell.edu/~ross/publications/mixedsite/)[时间](http://www.cs.cornell.edu/~ross/publications/chpeffects/)；这是我研究议程的一大部分。但是你必须小心应用它的地方。健全是一种安全保证，而不是可用性问题。重要的是是否有人能写这个，因为这样有人就能绕过人们信任的安全措施，做他们想做的任何事情。在健全的情况下，你应该担心的是恶意的程序员，而不仅仅是你和你的朋友。
...
`Foo<String>`作为有效类型安全吗？例如，下面的代码安全吗:
...
因此，根据同样的推理，`Constraint<U,? super T>`是完全安全有效的。只有在运行时`T`是`U`的子类型，满足约束条件时，才能拥有它的实例。
...
假设我们有一个类型为`Constrain<U,? super T>`的表达式 *e* 。此表达式的类型中有一个通配符。类型检查器不知道它代表什么，但是它确实代表了一些东西，所以类型检查器给这个未知的东西一个临时的名字，比如说`X`。此外，类型检查器知道未知的东西至少必须是`T`的超类型。因此，类型检查器将 *e* 视为具有类型`Constrain&llt;U,X>`，其中`X`是`T`的超类型。

但是类型检查器知道更多的东西。它知道`X`必须是有效的类型参数。因此，类型检查器也注意到`X`必须是`U`的子类型。我们称之为通配符上的*隐式*约束，而`super T`是通配符上的显式约束。我发现，尽管不常见，但这种隐式约束实际上被用于许多 Java 库的类型检查。论文给出了一个具体的例子来说明为什么隐式约束是重要的。
...
好吧，如果不可靠型不是不健康的根源，那是什么？令人惊讶的是，空指针不仅会导致程序出错，还会导致类型系统出错！通配符捕获的理由完全忘记了空指针。它假设对于某些`X`一定有一些实际的`Constrain<U,X>`，这个假设在对`X`的隐式约束中表现出来。所以所有这些都归结为一个空指针错误。但是与大多数空指针错误不同，这个错误花了 12 年才被发现。哦，它也不像大多数空指针错误那样容易修复，因为涉及到的每个特性都在实践中使用。但是 Java 团队正在解决这个问题。在你再次坚持解决方案是不允许使用`Constrain<U,? super T>`类型之前，要意识到这种推理将核心 Scala 的可靠性证明(没有 null)推迟了整整十年，我并不特别渴望花十年时间来解决这个问题。
