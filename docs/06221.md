...
> 原文：<https://dev.to/rogerjin12/5-benefits-of-immutable-objects-worth-considering-for-your-next-project>

*想了解更多类似的内容，请在推特上关注 [ButterCMS，并订阅我们的](https://twitter.com/ButterCMS)[博客](https://buttercms.com/blog/)。*

当第一次学习面向对象编程(OOP)时，通常会创建一个非常基本的对象，并实现 getters 和 setters。从那时起，对象就是这个可塑数据的神奇世界。然而，您会惊奇地发现，有时去掉改变对象中数据的能力会使代码更加简单易懂。不可变对象就是这种情况。
...
PHP 特别不支持开箱即用的线程，所以在使用这种语言时，这种好处并不明显。
...
```
create(new SimplePerson(â€˜Joe’, 42));
```
...
此外，当出现任何问题时，调试会更容易。你可以确定对象被给你的状态没有改变，因此更容易找到错误的源头。
...
*此文原帖[此处](https://buttercms.com/blog/5-things-you-should-know-about-immutable-objects)。*