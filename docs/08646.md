...
[![OpenAPI3 structure](../Images/4c21beaf0c5a81c8329fec7f8b2a15cd.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--ieF6SHAc--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://www.openapis.org/wp-content/uploads/2017/03/image00-1-768x539.png)
...
## [](#1-parameters-dont-have-a-type-property-any-more)1。参数不再有类型属性
...
## [](#2-headers-dont-have-a-type-property-any-more)2。标头不再有类型属性

好吧，这有点作弊。正如我上面所说的，OpenAPI 3 规范的目标是一致性，这就是我们这里所拥有的。要定义一个标头的类型~~而不是无格式字符串，只需~~您必须声明或重用一个`schema`或`content`对象。

## [](#3-oauth2-authentication-now-supports-multiple-flows)3。oAuth2 身份验证现在支持多个流
...
## [](#4-terms-of-service-must-now-be-a-url)4。服务条款现在必须是一个 URL
...
## [](#5-rules-on-reusable-component-names)5。关于可重用组件名的规则
