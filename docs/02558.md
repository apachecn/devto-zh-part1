...
#### [](#download-the-package-from-nuget)从 Nuget 下载软件包

使用软件包管理器控制台`Install-Package Ninject -Version 3.3.4`

使用点网命令行界面`dotnet add package Ninject --version 3.3.4`

#### [](#add-these-members-to-raw-startupcs-endraw-class-as-shown-below)将这些成员添加到`Startup.cs`类，如下所示
...
#### [](#add-the-following-binding-in-the-end-of-raw-configureservices-endraw-raw-startupcs-endraw-)在`ConfigureServices` ( `Startup.cs`)的末尾增加以下绑定

`services.AddSingleton<IHttpContextAccessor, HttpContextAccessor>();`

#### [](#create-class-raw-requestscopingstartupfilter-endraw-implementing-the-raw-istartupfilter-endraw-interface)创建类`RequestScopingStartupFilter`实现`IStartupFilter`接口
...
#### [](#create-a-static-class-raw-aspnetcoreextensions-endraw-with-the-following-extension-method)用下面的扩展方法创建一个静态类`AspNetCoreExtensions`
...
#### [](#configure-to-use-the-middleware-in-raw-configureservices-endraw-in-the-end-of-the-method)配置使用`ConfigureServices`中的中间件(在方法的最后)

`services.AddRequestScopingMiddleware(() => scopeProvider.Value = new Scope());`

#### [](#create-a-file-raw-activatorscs-endraw-with-the-following)用以下内容创建一个文件`Activators.cs`
...
#### [](#add-the-following-extension-method-to-raw-aspnetcoreextensionscs-endraw-)添加以下扩展方法到`AspNetCoreExtensions.cs`
...
#### [](#append-the-following-to-the-end-of-raw-configureservices-endraw-)在`ConfigureServices`的末尾添加以下内容

`services.AddCustomControllerActivation(Resolve);`

#### [](#add-another-class-to-raw-activatorscs-endraw-)添加另一个类到`Activators.cs`
...
#### [](#and-another-extension-method-in-raw-aspnetcoreextensionscs-endraw-)和`AspNetCoreExtensions.cs`中的另一种扩展方法
...
#### [](#create-an-raw-applicationbuilderextensionscs-endraw-with-a-static-class-in-it)创建一个包含静态类的`ApplicationBuilderExtensions.cs`
...
#### [](#create-a-raw-testcontroller-endraw-to-see-if-our-di-works)创造一个`TestController`看看我们的 DI 是否管用
...
#### [](#i-have-compiled-this-article-from-this-answer-on-stackoverflow-with-a-little-help-from-this-dotnetjunkie-repo)我在这个[网虫](https://github.com/dotnetjunkie/Missing-Core-DI-Extensions)回购的一点帮助下，根据[在 stackoverflow 上的这个](https://stackoverflow.com/questions/46693305/how-to-integrate-ninject-into-asp-net-core-2-0-web-applications)回答整理了这篇文章。