...
> 原文:[https://dev.to/mizutani/dns-gazer-4c8n](https://dev.to/mizutani/dns-gazer-4c8n)
...
### [](#use-case-1-sending-dns-transaction-logs-from-network-interface-raw-eth0-endraw-to-fluentd-on-raw-localhost-endraw-and-port-raw-24224-endraw-)用例 1:将 DNS 事务日志从网络接口`eth0`发送到`localhost`上的 fluentd 和端口`24224`
...
### [](#use-case-2-printing-dns-transaction-logs-from-raw-eth0-endraw-to-stdout)用例 2:从`eth0`到 stdout 打印 DNS 事务日志
...
### [](#use-case-3-printing-not-only-dns-transaction-logs-but-also-record-logs-from-raw-eth0-endraw-to-stdout)用例 3:不仅打印 DNS 事务日志，还打印从`eth0`到 stdout 的“记录日志”
...
## [](#setup-docker-image)设置(Docker 图像)
...
## [](#setup-build-source-code)设置(构建源代码)
