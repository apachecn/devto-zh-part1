# 将您的第一张图片发布到 Docker Hub

> 原文：<https://dev.to/rusrushal13/publish-your-first-image-to-docker-hub>

正如你所熟悉的 Docker 从我以前的职位。让我们深入探索更多。
...
我们在这里做的是将端口 4000 映射到容器公开端口 80。您应该会在 http://0.0.0.0:80 看到 Python 正在为您的应用程序提供服务的通知。但是该消息来自容器内部，它不知道您将该容器的端口 80 映射到 4000，使得 URL[http://localhost:4000](http://localhost:4000)。在 web 浏览器中访问该 URL，查看网页上显示的内容，包括“Hello World”文本和容器 ID。
...
*   标记图像:这更像是命名图像的版本。这是可选的，但是推荐使用，因为它有助于维护版本(与 ubuntu:16.04 和 ubuntu:17.04 相同)

```
docker tag imagebuildinginprocess rusrushal13/get-started:part1 
```

*   发布图像:将您标记的图像上传到存储库:一旦完成，上传结果将公开。如果你登录到 Docker Hub，你会在那里看到新的图像，以及它的 pull 命令。

```
docker push rusrushal13/get-started:part1 
```

对，就这样，你完了。现在你可以去码头枢纽，也可以检查它；).你发表了你的第一张照片。

我发现这个 GitHub 库真的很棒。看看吧[https://github.com/jessfraz/dockerfiles](https://github.com/jessfraz/dockerfiles)
